...
class FacePredictor():
    ...
    def update_record(self, db_connection, name):
        import datetime

        # using now() to get current time
        current_time = datetime.datetime.now()
        update_statement = """Update AttendanceMaster.Attendance set Attendance_Status = %s, Attendance_Date = %s WHERE student_name = %s"""
        values = (1, current_time, name)
                ...
                if len(bboxes) != 0:
                    ...
                    for bboxe in bboxes:
                        ...
                        if cos_similarity < cosine_threshold and proba > proba_threshold:
                            ...
                            self.update_record(db_connection,name)
